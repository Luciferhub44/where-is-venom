# Where Is Venom? — Developer Guide

Technical setup, deployment, and configuration for the [whereisvenom.com](https://whereisvenom.com) codebase. For what this project *is*, see [README.md](README.md).

A donations + fundraising site for Queen Xtelle's "Where Is Venom?" story, built with Next.js (App Router) and Paystack, deployed on Vercel.
...
- **"2,000 Cups for Venom"** campaign section: a 3-day, 2,000-cup fundraising
  ...
  bar and countdown timer, product details, and a handmade/dispatch note.
  Sponsoring checks out directly with name, phone, and shipping address.
- **Donations**: quick-donate tiers + custom amount, in NGN or USD (USD is
  derived from NGN via a fixed FX rate in `lib/currency.ts`).
...
There is no merch shop or cart in this version — it was removed. `app/cart`
just redirects to `/`. If you want it back later, the previous build had a
`lib/products.ts` catalog and a `CartProvider` you can reintroduce as a
starting point.
...
| `NEXT_PUBLIC_SITE_URL` | your production URL, e.g. `https://whereisvenom.com` |
...
### Domain setup notes (apex vs www)

If both `whereisvenom.com` and `www.whereisvenom.com` are added in Vercel,
one of them 308-redirects to the other (whichever is set as primary under
**Settings → Domains**). This matters for anything that fetches a URL
directly and does *not* follow redirects — notably Apple Pay domain
verification (see below). Register the domain that actually serves content
directly (no redirect) with any such third-party verifier.
...
static number (`CAMPAIGN_FALLBACK_CUPS_SPONSORED` in `lib/campaign.ts`) and
won't persist between requests. To make it live:
...
The live counter is stored under the Redis key `wiv:cups_sponsored`.

## 6. Customize before launch

- **Campaign dates**: edit `CAMPAIGN_START_ISO` in `lib/campaign.ts` to your
  real launch date/time — the countdown is driven from this.
- **Cup price / claimed count**: `CUP_PRICE` and
  `CAMPAIGN_FALLBACK_CUPS_SPONSORED` in `lib/campaign.ts`.
- **Donation tiers**: `lib/donations.ts` — NGN amounts are canonical, USD is
  derived via the fixed FX rate in `lib/currency.ts` (`NGN_PER_USD`).
...
Every successful donation and cup purchase gets a branded confirmation
email, sent from the webhook via [Resend](https://resend.com) once
`charge.success` fires — right after the transaction is logged in step 7.
...
Paystack's Apple Pay domain verification requires a static file at
`https://<your-domain>/.well-known/apple-developer-merchantid-domain-association`
— it lives in this repo at `public/.well-known/` (must stay under `public/`
or Next.js won't serve it), and `next.config.js` sets its `Content-Type` to
`text/plain` explicitly since extensionless files default to
`application/octet-stream` otherwise. See the domain setup note in step 3 —
Apple's verifier does not follow redirects, so register whichever domain
(apex or `www`) actually serves content directly.

## 10. Analytics & SEO

- [Vercel Analytics](https://vercel.com/analytics) is wired in via
  `<Analytics />` in `app/layout.tsx` — view it in the Vercel dashboard's
  **Analytics** tab once deployed and receiving traffic.
...
charge. The Redis-backed transaction log (and the `/admin` dashboard that
reads it) is a convenience layer on top of that, not a replacement — if you
ever need a full relational database, add one (Vercel Postgres, Supabase,
etc.) and write records inside `app/api/paystack/webhook/route.ts`, where
`charge.success` is already being handled.
...
lib/                             Episodes, campaign config, currency, Paystack + Redis + Resend helpers, admin auth
